...
Legato tokens provide a secure method of taking payments that reduces your PCI scope. You can take a payment using a token the same as you would take a payment with a credit card, the main difference being the ‘payment_method’ parameter and supplying the Legato token.

Before processing a transaction using the "Legato Token" method, you need to have created a token (see Legato).

A token is a "single-use nonce". Please distinguish it from a multi-use Payment Profile token (see Payment Profiles).
